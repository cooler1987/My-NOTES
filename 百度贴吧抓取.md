...
### 要点
1，编写的sh或py文件要用Unix 换行符，而不是 Windows 的换行符，必须使用UTF-8 编码，服务器要开TCP端口一个，如11999 

2，脚本名“web_scraper.py”，可能要用的命令，保持程序后台工作：`nohup python3 web_scraper.py &`，查看进程ID，`grep web_scraper.py`，中止进程：`kill ID值`
